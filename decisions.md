...
## 2026-08-26 — Social proof Beranda memakai ilustrasi alur, kartu statistik ditunda
**Keputusan:** Section social proof (SPEC §5.1.2) diisi blok **ilustrasi alur** berlabel eksplisit ("CS ditangani manual" vs "CS dibantu AI agent") + pernyataan terbuka bahwa belum ada klien/testimoni. Kartu statistik riset industri ditunda ke sesi tersendiri, menunggu angka **dan** sumber yang bisa diverifikasi.
**Ditolak:** Memasang kerangka kartu statistik berisi `TODO:` di halaman produksi.
**Alasan:** SPEC §5.1.2 merestui dua bentuk — statistik riset bersumber, atau blok ilustrasi berlabel. Yang tidak tersedia sekarang cuma bentuk pertama: sesi ini tidak punya angka yang bisa diverifikasi, dan mengarangnya melanggar SPEC §2.8. Opsi kartu ber-`TODO:` sebenarnya jujur, tapi menayangkan slot kosong yang kelihatan di halaman paling depan — biaya kredibilitas yang tidak perlu dibayar ketika ada bentuk lain yang sama-sama sah dan halamannya jadi utuh. Ilustrasi juga bukan klaim: keduanya deskripsi cara kerja, bukan hasil pengukuran.
**Konsekuensi:** Saat sumber statistik tersedia, tambahkan di `src/sections/SocialProof.astro` (komentar di file itu sudah menandainya) — bukan bikin section baru.

## 2026-09-14 — Kotak gabungan di SPEC §13 dipecah per unit kerja
**Keputusan:** Poin §13 yang memayungi beberapa halaman/aset ("Semua halaman ada" = 6 halaman; "robots.txt, favicon, og-image"; "Semua persyaratan SEO §8") dipecah jadi satu kotak per unit kerja. Ditambah aturan di CLAUDE.md: kotak kosong ≠ belum dikerjakan — cek `ls src/pages src/sections public` sebelum melapor.
**Ditolak:** (a) Mencentang kotak gabungan meski sebagian belum jadi — bohong; (b) membiarkan seperti semula dan mengandalkan tiap sesi membaca kode — sudah terbukti bocor.
**Alasan:** Dua kali berturut-turut brief pagi melaporkan Layanan/Artikel/Kontak/404 "belum dikerjakan", padahal semuanya sudah ter-merge sejak PR #4–#11. Penyebabnya bukan lupa mencentang: kotak "Semua halaman ada" memang sengaja dibiarkan kosong (2026-08-30, 2026-09-11) karena Tentang menunggu data pendiri — tapi satu kotak untuk 6 halaman tidak punya cara jujur menyatakan "5 dari 6 selesai", jadi 5 halaman yang sudah jadi ikut terlaporkan belum ada. Sisi lain: brief itu hanya mem-grep kotak kosong tanpa membuka `src/pages/`. CLAUDE.md sudah menjaga arah "nama file ≠ selesai", tapi belum menulis arah sebaliknya. Preseden pemecahan sudah ada: poin deploy dipecah 3 di PR #3 dengan alasan yang sama. Syarat "data pendiri terisi" dipindah ke poin placeholder — itu pekerjaan data user, bukan kode, dan mencampurnya membuat kotak halaman merah selamanya di luar kendali agent.
**Pelajaran (kelima kalinya):** pola yang sama dengan empat blok gerbang sebelumnya — sesuatu yang tampak melapor padahal tidak. Tracker yang tidak bisa menyatakan status parsial sama menyesatkannya dengan gerbang yang selalu hijau.
